    ...
    with open(input_file, 'r', newline='') as infile, \
        ...
        for row in reader:
            try:
                ...
                row['Sensor Time'] = human_time  # Replace the "Time" value with the human-readable timestamp
...
def plotResults(file_path, timeType):
    ...
    axs[0].plot(df[timeType], df['Temperature'], label='Temperature', color='r')
    axs[0].set_title(f"Temperature vs {timeType}")
    axs[0].set_ylabel('Temperature (°C)')
    axs[0].legend()
    
    # Plot Pressure vs Time
    axs[1].plot(df[timeType], df['Pressure'], label='Pressure', color='b')
    axs[1].set_title(f"Pressure vs {timeType}")
    axs[1].set_ylabel('Pressure (Pa)')
    axs[1].legend()
    
    # Plot Humidity vs Time
    axs[2].plot(df[timeType], df['Humidity'], label='Humidity', color='g')
    axs[2].set_title(f"Humidity vs {timeType}")
